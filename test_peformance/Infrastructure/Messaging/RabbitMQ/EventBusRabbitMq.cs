...
public class EventBusRabbitMq : IEventBus, IDisposable
{
    ...
    public EventBusRabbitMq(IRabbitMqPersistentConnection persistentConnection,
        IServiceScopeFactory scopeFactory, IEventBusSubscriptionsManager subsManager, string queueName = null, int retryCount = 5)
    {
    ...
    public void Unsubscribe<T, TH>()
        ...
        where TH : IIntegrationEventHandler<T>
    {
        Log.Information("Unsubscribing from event {EventName}", _subsManager.GetEventKey<T>());
    ...
    private async Task ProcessEvent(string eventName, string message)
    {
        ...
        foreach (var subscription in subscriptions)
        {
            ...
            if (subscription.IsDynamic)
            {
                ...
                await Task.Yield();
                await ((IDynamicIntegrationEventHandler)handler).Handle(eventData);
            }
            else
            {
